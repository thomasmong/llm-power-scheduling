...
model = "llama3-classifier-scenar1-2"
...
def ira_for_file(filepath, classifier: ClassifierAgent):
    ...
    output_path = os.path.join('ira/scenar1/', output_file)
    ...
    for problem in dict_problems[op_id]:
        ...
        for file in os.listdir(dir):
            if file.startswith(problem):
                ...
                if not os.path.exists(os.path.join('ira/scenar1/',file[:-4] + f"-ira-{model}-{max_classes}.txt")):
